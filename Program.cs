...
void HandleEnemyDefeat(Player currentPlayer, Enemy currentEnemy, Random rng, Location currentLocation)
{
    ...
    if (enemiesDefeated == 3)
    {
        ...
        {
            Bosses.WhisperingEntBoss boss = new Bosses.WhisperingEntBoss();
            Console.WriteLine($"You have chosen to fight the boss: {boss.Name}!");
            Console.WriteLine($"{boss.Description}");
            BattleResult bossResult = RunBattle(currentPlayer, new Enemy(boss.Name, boss.Health, boss.Attack, boss.Armor) { XpReward = 100, GoldReward = 200 }, rng, currentLocation);
